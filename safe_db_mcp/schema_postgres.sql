...
-- Postgres dialect. The data is identical to schema.sql; only the DDL differs.
-- Applied explicitly by `python -m safe_db_mcp.seed_postgres`, never automatically:
-- a server that runs DDL on connect is exactly what this project argues against.
...
-- Grants live here on purpose: the reader role holding SELECT and nothing else
-- is the Postgres equivalent of the SQLite backend's mode=ro flag, and it is
-- the stronger of the two because the server enforces it against a login that
-- has no other privilege.
