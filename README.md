...
**An [MCP](https://modelcontextprotocol.io) (Model Context Protocol) server: guardrails on a real database, not just a demo file.** Give an agent read/write access and take away its ability to wreck the data - whether that database is a local SQLite file or **Postgres on AWS RDS**.
